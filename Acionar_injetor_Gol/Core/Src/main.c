...
/*
 ...
 adcBuffer[0] --> Temp. Agua
 adcBuffer[0] --> Pedal pista 1
 adcBuffer[0] --> Pedal pista 2
 */
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) //chamdada da interrupção externa dente da roda fonica
{
	contDenteFonica = __HAL_TIM_GET_COUNTER(&htim2); // Lê o valor atual do contador
	 ...
	 else
	 {
		 ...
		 {
			 tempoInjetor = 4; // PRECISA COLOCAR FORMULA --> num de contadores = tempo_injecao_calculado(em seg) / 0.00001 seg
			// HAL_GPIO_WritePin
			 HAL_TIM_Base_Start_IT(&htim3);
			  __HAL_TIM_DISABLE(&htim3);          // Desabilita o timer
			  __HAL_TIM_SET_AUTORELOAD(&htim3, tempoInjetor - 1); // Novo período do tempo de injeção
